...
import { useOscilloscopeData } from "@/hooks/use-oscilloscope-data";
...
export type OscilloscopeProps = HTMLAttributes<HTMLDivElement> & {
  ...
  lineColor?: string;
  /** Start color for horizontal gradient (left). Use hex/rgb; CSS variables are not supported by canvas. */
  gradientStartColor?: string;
  /** End color for horizontal gradient (right). Use hex/rgb; CSS variables are not supported by canvas. */
...
export const Oscilloscope = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (useGradient) {
        const gradient = ctx.createLinearGradient(0, centerY, rect.width, centerY);
